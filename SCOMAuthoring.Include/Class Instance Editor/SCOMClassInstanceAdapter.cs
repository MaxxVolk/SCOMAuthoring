...
namespace SCOMAuthoringBook.Library.Editor
{
  public abstract class SCOMClassInstanceAdapter : INotifyPropertyChanged, IEditableObject
  {
    ...
    protected void PropertyChangedInvoke([CallerMemberName]string propertyName = "")
    {
      ...
      if (propertyName != "CommitStatus") // to avoid infinite loop and commit status change should not change commit status
    ...
    public BindableMonitoringObject BindableActionPoint
    {
      ...
      set { ActionPoint = value.MonitoringObject; } // no property change notification is over here, as it's just a proxy to ActionPoint
    }
